...
function isAtomNode(node) {
  return node.type in atomRegistry;
}


/** @private */
function isMoleculeNode(node) {
  return node.type in moleculeRegistry;
}
...
function numAtoms(jkif) {
  validateJkif(jkif);
  var atoms = 0;
  eachNode(jkif, function(node) { if (isAtom(node)) atoms++; });
  return atoms;
}
...
function numMolecules(jkif) {
  validateJkif(jkif);
  var molecules = 1;
  eachNode(jkif, function(node) { if (isMolecule(node)) molecules++; });
  return molecules;
}
