    ...
    if (newNodes.length) {
      switch (newNodes[0]) {
        ...
        case 'conjunction':
          ...
          break;
        case 'disjunction':
          var disjunctionNodes = this.createDisjunctionNodes(newNodes[1], node.line);
          this.manageBranchingStacks(this.cloneAllOpenStacks(), disjunctionNodes);
          break;
        case 'implication':
          var implicationNodesTuple = this.createImplicationNodes(newNodes[1], node.line);
          this.manageBranchingStacks(this.cloneAllOpenStacks(), implicationNodesTuple);
          break;
        case 'equivalence':
          break;
        default:
 ...
 * @params {Array} disjunctionNodes
 */
function manageBranchingStacks(clonedStacks, nodesToAdd) {
...
function createNegationNode(nodeData, rLine) {
  var newNode = new Node(nodeData, this.STEP_TACTUS++, { fromLine: rLine, name: 'negationDecomposition' });
  this.eachOpenStack(function(stack) {
    stack.forEach(function(node) {
      if (_.eqDeep(nodeData, node.proposition)) {
        newNode._id = node._id > 0 ? -node._id : node._id;
      }
    });
  });
  return [newNode];
  ...
  this.eachOpenStack(function(stack) {
    stack.forEach(function(node) {
      if (_.eqDeep(antecedent.proposition, node.proposition)) {
        antecedent._id = node._id > 0 ? -node._id : node._id;
        matched = true;
      }
    });
  });
  if (!matched) {
    tuple[0]._id = antecedent._id > 0 ? -antecedent._id : antecedent._id;
  }
  return tuple;
}
