      ...
      switch (newNodeType) {
        case 'negation':
          ...
          if (stackTest) {
            ...
            var leftBranch = negationReturn.slice(1, 4), rightBranch = negationReturn.slice(1);
            rightBranch.splice(1, 2);
            this.manageBranchingDoubleStacks([leftBranch, rightBranch]);
          }
          break;
        case 'conjunction':
...
function manageBranchingDoubleStacks(branchingNodes) {
  ...
  branchingNodes[1].forEach(function(node) {
    this.addNodeToAllClonedStacks(clonedStacks, node);
  }.bind(this));
  this.addAllClonedStacksToOpenStacks(clonedStacks);
}
...
function createNegationNode(nodeData, rLine) {
  ...
  switch (nodeData.type) {
    ...
    case 'ImplicationNode':
      newNodes.push('negatedImplication');
      var newImplicationNode = new Node(nodeData, this.STEP_TACTUS++, { fromLine: rLine, name: 'negatedImplicationDecomposition' });
      var newAntecedentNode = new Node(newImplicationNode.proposition.antecedent, this.STEP_TACTUS++, { fromLine: rLine, name: 'negatedImplicationDecomposition' });
      var newConsequentNode = new Node(newImplicationNode.proposition.consequent, this.STEP_TACTUS++, { fromLine: rLine, name: 'negatedImplicationDecomposition' });
      newImplicationNode.checked = true;
      newNodes.push(newImplicationNode, newAntecedentNode, newConsequentNode);
      var matchedAnt = false, matchedConsq = false;
      this.eachOpenStack(function(stack) {
        stack.forEach(function(node) {
          if (_.eqDeep(newAntecedentNode.proposition, node.proposition)) {
            matchedAnt = true;
            newNodes[2]._id = Math.abs(node._id);
          }
          if (_.eqDeep(newConsequentNode.proposition, node.proposition)) {
            newNodes[3]._id = -Math.abs(node._id);
            matchedConsq = true;
          }
        });
      });
      if (!matchedAnt) newNodes[2]._id = Math.abs(newAntecedentNode._id);
      if (!matchedConsq) newNodes[3]._id = -(Math.abs(newConsequentNode._id));
      return newNodes;
      break;
    case 'EquivalenceNode':
      newNodes.push('negatedEquivalence');
      var newEquivalenceNode = new Node(nodeData, this.STEP_TACTUS++, { fromLine: rLine, name: 'negatedEquivalenceDecomposition' });
      var newEquivNodeLA = new Node(newEquivalenceNode.proposition.expressions[0], this.STEP_TACTUS, { fromLine: rLine, name: 'negatedEquivalenceDecomposition' });
      var newEquivNodeRA = new Node(newEquivalenceNode.proposition.expressions[0], this.STEP_TACTUS++, { fromLine: rLine, name: 'negatedEquivalenceDecomposition' });
      var newEquivNodeLB = new Node(newEquivalenceNode.proposition.expressions[1], this.STEP_TACTUS, { fromLine: rLine, name: 'negatedEquivalenceDecomposition' });
      var newEquivNodeRB = new Node(newEquivalenceNode.proposition.expressions[1], this.STEP_TACTUS++, { fromLine: rLine, name: 'negatedEquivalenceDecomposition' });
      newEquivalenceNode.checked = true;
      newNodes.push(newEquivalenceNode, newEquivNodeLA, newEquivNodeLB, newEquivNodeRA, newEquivNodeRB);
      var matchedLA = false, matchedLB = false, matchedRA = false, matchedRB = false;
      this.eachOpenStack(function(stack) {
        stack.forEach(function(node) {
          if (_.eqDeep(newEquivNodeLA.proposition, node.proposition)) {
            newNodes[2]._id = Math.abs(node._id);
            matchedLA = true;
          }
          if (_.eqDeep(newEquivNodeLB.proposition, node.proposition)) {
            newNodes[3]._id = -Math.abs(node._id);
            matchedLB = true;
          }
          if (_.eqDeep(newEquivNodeRA.proposition, node.proposition)) {
            newNodes[4]._id = -Math.abs(node._id);
            matchedRA = true;
          }
          if (_.eqDeep(newEquivNodeRB.proposition, node.proposition)) {
            newNodes[5]._id = Math.abs(node._id);
            matchedRB = true;
          }
        });
      });
      if (!matchedLA) newNodes[2]._id = Math.abs(newEquivNodeLA._id);
      if (!matchedLB) newNodes[3]._id = -(Math.abs(newEquivNodeLB._id));
      if (!matchedRA) newNodes[4]._id = -Math.abs(newEquivNodeRA._id);
      if (!matchedRB) newNodes[5]._id = (Math.abs(newEquivNodeRB._id));
      return newNodes;
      break;
    default:
